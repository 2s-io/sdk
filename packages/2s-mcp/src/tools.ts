...
export function buildToolList(c: TwoS): ToolDef[] {
  ...
  const t: ToolDef[] = [
    ...
    },
    {
      ...
      invoke: (a) => c.watchers.cancel(a as never),
    },
    {
      name: 'watchers.crypto-address-activity',
      description: 'WATCHER: get a signed callback the moment a crypto address transacts. Arm once, pay once (no account, no API key) — we watch Base, Ethereum, or Bitcoin and POST your custom payload to callbackUrl when the address sends/receives native coins, ERC-20s, or ERC-721s. Filter by direction (in/out/both), asset type, and a USD',
      inputSchema: s('crypto-address-activity', {
        chain: { type: 'string', description: 'Chain.' },
        address: { type: 'string', description: 'Street address or full address string.' },
        callbackUrl: { type: 'string', description: 'Any https(s) URL; your payload is POSTed here on each event.' },
        direction: { type: 'string', description: 'Direction.' },
        assetTypes: { type: 'object', description: 'Asset types.' },
        minValueUsd: { type: 'number', description: 'Min value usd.' },
        payload: { type: 'object', description: 'Payload.' },
        expiresInSeconds: { type: 'integer', description: 'Expires in seconds.' },
        maxFires: { type: 'integer', description: 'Max fires.' },
        label: { type: 'string', description: 'Label.' },
      }, ['chain', 'address', 'callbackUrl']),
      invoke: (a) => c.watchers.cryptoAddressActivity(a as never),
    },
    {
      name: 'watchers.status',
      description: 'Status of a watcher by watcherId: state (armed/completed/expired/cancelled), fires used/remaining, expiry, recent deliveries (with HTTP result + attempt count), and any UNDELIVERED events with their full callback bodies — the pull backstop, so a missed push is always recoverable here. Pairs with watchers.crypto-address',
    ...
    {
      name: 'watchers.token-price',
      description: 'WATCHER: get a signed callback when a crypto asset crosses a price you set. Arm once, pay once (no account, no API key) — we poll the spot price and POST your custom payload to callbackUrl the moment the condition is met. tokenId is a CoinGecko asset id (lowercase, e.g. bitcoin, ethereum, solana — not the ticker). conditionType: \'above\' / \'below\' (threshold = a USD price) or \'pct_up\' / \'pct_down\' ',
      inputSchema: s('token-price', {
        tokenId: { type: 'string', description: 'CoinGecko asset id (lowercase, not ticker), e.g. bitcoin, ethereum, solana.' },
        conditionType: { type: 'string', description: 'above/below = threshold is a USD price; pct_up/pct_down = threshold is a percent move over 24h.' },
        threshold: { type: 'number', description: 'Trigger level: a USD price for above/below (e.g. 100000), or a percent for pct_up/pct_down (e.g. 10 = ±10% over 24h).' },
        callbackUrl: { type: 'string', description: 'Where we POST the event. Any http(s) URL; the JSON body is signed (verify with X-2s-Signature). e.g. https://your-agent.app/hooks/btc' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back verbatim in every callback. e.g. {"agentId":"a1"}' },
        expiresInSeconds: { type: 'integer', description: 'How long the watch stays active, in seconds. Default 2592000 (30 days), max 7776000 (90 days).' },
        maxFires: { type: 'integer', description: 'Stop after this many callbacks (default 1 — a one-shot alert). Each fire is a fresh crossing into the condition.' },
        label: { type: 'string', description: 'Optional free-text tag to recognize this watcher later.' },
      }, ['tokenId', 'conditionType', 'threshold', 'callbackUrl']),
      invoke: (a) => c.watchers.tokenPrice(a as never),
    },
    {
      name: 'watchers.gas-price',
      description: 'WATCHER: get a signed callback when EVM gas crosses a level you set — e.g. "wake me when Ethereum gas drops below 10 gwei." Arm once, pay once (no account, no API key). chain: base | ethereum | polygon | arbitrum | optimism. conditionType: \'below\' or \'above\'; threshold is in gwei (compared to the chosen fee tier\'s max fee per gas). tier: slow | standard | fast (default standard). Fires once per cr',
      inputSchema: s('gas-price', {
        chain: { type: 'string', description: 'EVM chain whose gas to watch.' },
        conditionType: { type: 'string', description: '\'below\' (cheap gas alert) or \'above\' (congestion alert).' },
        threshold: { type: 'number', description: 'Gas price threshold in gwei (e.g. 10), compared to the chosen tier’s max fee per gas.' },
        tier: { type: 'string', description: 'Fee tier to compare against. Default: standard.' },
        callbackUrl: { type: 'string', description: 'Where we POST the event. Any http(s) URL; the JSON body is signed (verify with X-2s-Signature). e.g. https://your-agent.app/hooks/gas' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back verbatim in every callback. e.g. {"agentId":"a1"}' },
        expiresInSeconds: { type: 'integer', description: 'How long the watch stays active, in seconds. Default 2592000 (30 days), max 7776000 (90 days).' },
        maxFires: { type: 'integer', description: 'Stop after this many callbacks (default 1). Each fire is a fresh crossing into the condition.' },
        label: { type: 'string', description: 'Optional free-text tag to recognize this watcher later.' },
      }, ['chain', 'conditionType', 'threshold', 'callbackUrl']),
      invoke: (a) => c.watchers.gasPrice(a as never),
    },
    {
      name: 'watchers.business-earnings',
      ...
      inputSchema: s('business-earnings', {
        ticker: { type: 'string', description: 'US-listed ticker to watch, e.g. AAPL.' },
      ...
      invoke: (a) => c.watchers.businessEarnings(a as never),
    },
    {
      name: 'watchers.fx-rate',
      description: 'WATCHER: get a signed callback when an FX pair crosses a rate you set. Arm once, pay once. base + quote are 3-letter ISO currency codes (e.g. base USD, quote EUR). conditionType \'above\'/\'below\'; threshold is the quote-per-base rate. Fires once per crossing; bounded by maxFires/expiry. Rates via Frankfurter (ECB). Signed (verify offline) + retried; recoverable via watchers.status. Returns a watcher',
      inputSchema: s('fx-rate', {
        base: { type: 'string', description: 'Base currency, e.g. USD.' },
        quote: { type: 'string', description: 'Quote currency, e.g. EUR.' },
        conditionType: { type: 'string', description: 'Fire when the rate goes above or below the threshold.' },
        threshold: { type: 'number', description: 'Quote-per-base rate to cross, e.g. 0.95.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many callbacks (default 1).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['base', 'quote', 'conditionType', 'threshold', 'callbackUrl']),
      invoke: (a) => c.watchers.fxRate(a as never),
    },
    {
      name: 'watchers.funding-rate',
      ...
      inputSchema: s('funding-rate', {
        coin: { type: 'string', description: 'Perp coin symbol, e.g. BTC, ETH, SOL.' },
        conditionType: { type: 'string', description: 'Fire when hourly funding goes above or below the threshold.' },
        threshold: { type: 'number', description: 'Hourly funding rate to cross (can be negative, e.g. -0.0001).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many callbacks (default 1).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['coin', 'conditionType', 'threshold', 'callbackUrl']),
      invoke: (a) => c.watchers.fundingRate(a as never),
    },
    {
      name: 'watchers.prediction-market',
      ...
      inputSchema: s('prediction-market', {
        conditionId: { type: 'string', description: 'Polymarket market conditionId (from predict.market / predict.markets).' },
        outcomeIndex: { type: 'integer', description: 'Which outcome to watch (0 = first, usually Yes). Default 0.' },
        conditionType: { type: 'string', description: 'Fire when the probability goes above or below the threshold.' },
        threshold: { type: 'number', description: 'Probability 0–1 to cross, e.g. 0.8.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many callbacks (default 1).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['conditionId', 'conditionType', 'threshold', 'callbackUrl']),
      invoke: (a) => c.watchers.predictionMarket(a as never),
    },
    {
      name: 'watchers.sec-filing',
      ...
      inputSchema: s('sec-filing', {
        ticker: { type: 'string', description: 'US-listed ticker, e.g. AAPL.' },
        form: { type: 'string', description: 'Optional filing type to filter on (e.g. 8-K, 10-K, 13F, 4). Omit for all forms.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many filings (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['ticker', 'callbackUrl']),
      invoke: (a) => c.watchers.secFiling(a as never),
    },
    {
      ...
      inputSchema: s('company-news', {
        ticker: { type: 'string', description: 'US-listed ticker, e.g. AAPL.' },
        keyword: { type: 'string', description: 'Optional: only fire on headlines containing this text (case-insensitive).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many articles (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['ticker', 'callbackUrl']),
      invoke: (a) => c.watchers.companyNews(a as never),
    },
    {
      name: 'watchers.ipo',
      ...
      inputSchema: s('ipo', {
        keyword: { type: 'string', description: 'Optional: only fire when the IPO name contains this text (case-insensitive).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many IPOs (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['callbackUrl']),
      invoke: (a) => c.watchers.ipo(a as never),
    },
    {
      name: 'watchers.federal-register',
      description: 'WATCHER: get a signed callback when a new US Federal Register document is published. Arm once, pay once. Optionally filter by type (RULE / PRORULE / NOTICE / PRESDOCU), agency (slug, e.g. environmental-protection-agency), and/or keyword in the title. Fires once per new document (deduped by document number); bounded by maxFires/expiry. Existing docs at arm time are baselined. Free public-domain dat',
      inputSchema: s('federal-register', {
        type: { type: 'string', description: 'Document type filter. Omit for all.' },
        agency: { type: 'string', description: 'Agency slug, e.g. federal-trade-commission.' },
        keyword: { type: 'string', description: 'Optional: only fire on titles containing this text (case-insensitive).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many documents (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['callbackUrl']),
      invoke: (a) => c.watchers.federalRegister(a as never),
    },
    {
      ...
      inputSchema: s('weather-alert', {
        area: { type: 'string', description: 'US state/territory code, e.g. CA, TX, FL.' },
        severity: { type: 'string', description: 'Optional: only fire on alerts of this severity.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many alerts (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['area', 'callbackUrl']),
      invoke: (a) => c.watchers.weatherAlert(a as never),
    },
    {
      name: 'watchers.earthquake',
      ...
      inputSchema: s('earthquake', {
        lat: { type: 'number', description: 'Latitude of the center point.' },
        lon: { type: 'number', description: 'Longitude of the center point.' },
        radiusKm: { type: 'number', description: 'Search radius in km (default 500).' },
        minMagnitude: { type: 'number', description: 'Minimum magnitude to fire on (default 4).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many quakes (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['lat', 'lon', 'callbackUrl']),
      invoke: (a) => c.watchers.earthquake(a as never),
    },
    {
      name: 'watchers.flight-status',
      description: 'WATCHER: get a signed callback when a flight\'s status changes (e.g. Scheduled → Delayed → Departed → Landed). Arm once, pay once. Pass ident (airline flight designator like UAL1 / UA1, or a tail number). Fires on each status transition for the nearest instance; bounded by maxFires/expiry. The status at arm time is baselined (only changes fire). Data via FlightAware. Signed + retried; recoverable v',
      inputSchema: s('flight-status', {
        ident: { type: 'string', description: 'Flight designator (UAL1 / UA1) or tail number.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 7 days; flights are short-lived).' },
        maxFires: { type: 'integer', description: 'Stop after this many status changes (default 10).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['ident', 'callbackUrl']),
      invoke: (a) => c.watchers.flightStatus(a as never),
    },
    {
      name: 'watchers.package-release',
      ...
      inputSchema: s('package-release', {
        registry: { type: 'string', description: 'Package registry: npm or pypi.' },
        name: { type: 'string', description: 'Package name, e.g. react (npm) or requests (pypi).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many releases (default 10).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['registry', 'name', 'callbackUrl']),
      invoke: (a) => c.watchers.packageRelease(a as never),
    },
    {
      ...
      inputSchema: s('ioc-reputation', {
        ioc: { type: 'string', description: 'Indicator to watch: an IP address or domain.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many changes (default 10).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['ioc', 'callbackUrl']),
      invoke: (a) => c.watchers.iocReputation(a as never),
    },
    {
      ...
      inputSchema: s('http-headers', {
        url: { type: 'string', description: 'The URL to monitor, e.g. https://example.com.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many changes (default 10).' },
    ...
    },
    {
      ...
      invoke: (a) => c.watchers.dns(a as never),
    },
    {
      name: 'watchers.whois',
      ...
      invoke: (a) => c.watchers.whois(a as never),
    },
    {
      name: 'watchers.fear-greed',
      ...
      inputSchema: s('fear-greed', {
        conditionType: { type: 'string', description: 'Fire when the index goes above or below the threshold.' },
        threshold: { type: 'number', description: 'Index value 0–100 to cross (e.g. 20 = extreme fear, 80 = extreme greed).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many callbacks (default 1).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['conditionType', 'threshold', 'callbackUrl']),
      invoke: (a) => c.watchers.fearGreed(a as never),
    },
    {
      name: 'watchers.fred-series',
      ...
      inputSchema: s('fred-series', {
        seriesId: { type: 'string', description: 'FRED series id, e.g. DGS10, UNRATE, CPIAUCSL, FEDFUNDS.' },
        conditionType: { type: 'string', description: 'Fire when the latest value goes above or below the threshold.' },
        threshold: { type: 'number', description: 'The series value to cross, e.g. 5 (for DGS10 = 5% 10yr yield).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many callbacks (default 1).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['seriesId', 'conditionType', 'threshold', 'callbackUrl']),
      invoke: (a) => c.watchers.fredSeries(a as never),
    },
    {
      name: 'watchers.patent',
      ...
      inputSchema: s('patent', {
        query: { type: 'string', description: 'Search query — keywords, assignee, inventor, etc.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many patents (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['query', 'callbackUrl']),
      invoke: (a) => c.watchers.patent(a as never),
    },
    {
      name: 'watchers.paper',
      ...
      inputSchema: s('paper', {
        query: { type: 'string', description: 'Search query — keywords, author, or topic.' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many papers (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['query', 'callbackUrl']),
      invoke: (a) => c.watchers.paper(a as never),
    },
    {
      ...
      inputSchema: s('product-recall', {
        keyword: { type: 'string', description: 'Optional: only fire on recalls whose title contains this text (case-insensitive).' },
        callbackUrl: { type: 'string', description: 'Signed event POSTed here (verify X-2s-Signature). Any http(s) URL.' },
        payload: { type: 'object', description: 'Arbitrary JSON echoed back in every callback.' },
        expiresInSeconds: { type: 'integer', description: 'Active window in seconds (default 30d, max 90d).' },
        maxFires: { type: 'integer', description: 'Stop after this many recalls (default 25).' },
        label: { type: 'string', description: 'Optional free-text tag.' },
      }, ['callbackUrl']),
